...
- **Теги** — общая таксономия для событий и новостей (модель Tag в приложении core): фильтры, поиск, маркировка на витрине.
- **Спикеры** — база спикеров с привязкой к событиям и сегментам программы.
- **Программа мероприятия** — сегменты (доклады, блоки, перерывы) с временем, местом и спикерами; отдельного справочника «тематик» нет.
...
### 2.3. EventSegment (сегмент программы)
...
### 2.4. EventRegistration (регистрация на событие)
...
### 2.5. EventGallery (галерея события)
...
- Теги и спикеры выбираются в два списка (filter_horizontal).
- Slug подставляется из названия. Поля created_at, updated_at только для чтения.

### 3.3. Сегменты программы
