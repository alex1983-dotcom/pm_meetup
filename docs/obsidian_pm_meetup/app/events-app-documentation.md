...
Документ описывает модели и разделы админ-панели, связанные с мероприятиями PM.Meetup: тематики (аналитика), спикеры, события, расписание, регистрации и галереи.
...
- **Тематики и теги** — справочник тематик для внутренней аналитики (не для карточек на сайте) и общие теги для витрины (теги хранятся в приложении core).
...
### 2.1. EventTheme (тематика мероприятия)

Справочник для **внутренней аналитики** (срезы, отчёты, будущий раздел аналитики). На публичную карточку события не ориентирован — для витрины используются тип события и теги.

| Поле | Тип | Описание |
|------|-----|----------|
| name | CharField(100) | Название тематики |
| slug | SlugField(120), уникальный | URL-путь (подставляется из названия) |
| description | TextField | Заметки (необязательно) |
| order | PositiveIntegerField | Порядок в списках |
| created_at, updated_at | DateTimeField | Авто (из TimeStampedModel) |

Один **Event** может иметь несколько тематик (связь «многие ко многим», поле `themes`).
...
| themes | M2M → EventTheme | Тематики (аналитика) |
...
- Список: название, дата, время начала, тип, формат, статус, рекомендованное событие, лимит, дата создания. Статус и рекомендацию можно менять в списке.
- Фильтры: статус, тип события, формат, рекомендованное событие.
...
| Метод | URL | Описание |
|-------|-----|----------|
| GET | `/api/v1/events/themes/` | Список тематик (справочник для аналитики) |
| GET | `/api/v1/events/themes/<slug>/` | Тематика по slug |
