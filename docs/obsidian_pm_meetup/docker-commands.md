...
Одна команда заполняет все приложения тестовыми данными: теги, пользователи, события, новости, контент, страницы с блоками, материалы. Повторный запуск не создаёт дубликаты.

```bash
# Загрузить тестовые данные
docker-compose exec web python manage.py seed_data

# Очистить все данные приложений и загрузить заново (осторожно!)
docker-compose exec web python manage.py seed_data --clear
```

После `seed_data` будут созданы:
- **Пользователи:** admin@pm.meetup (пароль admin123), author@pm.meetup (author123), user@pm.meetup (user123)
- **API-ключ:** заголовок `X-API-KEY: test-api-key-12345` для доступа к API
- События (в т.ч. опубликованные и архивное), категории, спикеры, сегменты, галереи, регистрации
- Новости (опубликованная и черновик), партнёры, команда, настройки сайта, статичные страницы (О нас, Контакты), заявка на партнёрство
- Страница с блоками «Главная» (hero + FAQ), категории и карточки материалов

Локально (с активированным venv и настроенным DJANGO_SETTINGS_MODULE):  
`python manage.py seed_data` или `python manage.py seed_data --clear`
