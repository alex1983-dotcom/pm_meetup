...
- **ManyToMany:** EventTheme (`event.themes` ↔ `theme.events`), Tag (`event.tags` ↔ `tag.events`), Speaker (`event.speakers` ↔ `speaker.events`).
...
| **themes** | **ManyToManyField(EventTheme)** | **related_name='events', blank — тематики для аналитики** |
...
## 5. Тематики мероприятия (EventTheme)

| Поле | Тип | Описание |
|------|-----|----------|
| name | CharField(100) | Название тематики |
| slug | SlugField(120), unique | URL-путь |
| description | TextField, blank | Заметки |
| order | PositiveIntegerField | Порядок отображения в списках |
| created_at | DateTimeField | Дата создания |
| updated_at | DateTimeField | Дата последнего изменения |
| **events** | **обратная M2M (Event.themes)** | **related_name='events' — события с этой тематикой** |

**Связи (обратные):** Поле связи задаётся в Event (themes → EventTheme). Со стороны EventTheme доступно `theme.events`.
...
| **Event** | themes | M2M | EventTheme | `event.themes` ↔ `theme.events` | blank |
...
| **EventTheme** | — | обратная M2M | Event | `theme.events` | задано в Event.themes |
...
1. **User** — обязательно
2. **Event**, **EventTheme**, **EventRegistration** — ядро
