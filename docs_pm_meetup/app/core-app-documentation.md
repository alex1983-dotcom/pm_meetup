...
- **ApiKey** — ключи для доступа к REST API (`DocsOrApiKey`: заголовок `X-API-KEY` или `?key=`).
...
Поле `key` в админке доступно только для чтения: ключ генерируется автоматически при создании и его можно скопировать из карточки ключа.
...
Эндпоинты доступны по базовому префиксу `**/api/v1/core/`**. Документация Swagger: `**/api/docs/**`.
...
| Метод | URL                         | Описание                                            |
| ----- | --------------------------- | --------------------------------------------------- |
| GET   | `/api/v1/core/tags/`        | Список всех тегов (для фильтров событий и новостей) |
| GET   | `/api/v1/core/tags/<slug>/` | Детали тега по slug                                 |
