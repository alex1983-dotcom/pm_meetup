...
> **Соответствие коду:** `apps/users/`, `AUTH_USER_MODEL = 'users.User'`. Публичного REST API регистрации/входа **нет** — см. [auth-sessions.md](../auth-sessions.md).

Документ описывает модель пользователя и раздел админ-панели для управления пользователями в проекте PM.Meetup.
...
| **is_blocked** | BooleanField               | Нет (по умолчанию False)        | Метка в админке. **Не проверяется** при входе; для блокировки доступа используйте `is_active=False`. |
...
- **Участник (member)** — значение по умолчанию при создании пользователя через `create_user` (админка, shell, фикстуры).
...
Создаёт обычного пользователя (не суперадмина). Вызывается из админки, management-команд, shell, фикстур. **Публичного API**, которое вызывает этот метод, в проекте нет.

1. Проверяется наличие email.
2. Email нормализуется.
3. Создаётся `User` с переданным email и полями из `extra_fields`.
4. Пароль хешируется через `set_password()`.
5. Запись сохраняется в БД.

Роль не передаётся явно — подставляется **`member`** из модели.
...
- **`create_user`** — обычный пользователь, роль `member`.
- **`create_superuser`** — `is_staff`, `is_superuser`, роль `superadmin`; вызывается только из `createsuperuser` или явно в коде.
...
- **EventRegistration** — FK на пользователя; для API регистраций нужна **сессия** (см. [auth-sessions.md](../auth-sessions.md)).
- **NewsArticle.author** — FK на пользователя (`SET_NULL`).

Отдельного `apps/users/urls.py` в публичном API нет.
...
| Вопрос | Ответ |
|--------|-------|
| Чем входят в админку? | Email и пароль (`USERNAME_FIELD = "email"`). |
| Как создать пользователя сейчас? | Админка, `createsuperuser`, фикстуры — не публичная форма на сайте. |
| Роль «Участник» | Значение по умолчанию (`member`) при `create_user`. |
| Суперадминистратор | Только через `create_superuser` / `createsuperuser`. |
| Блокировка доступа | `is_active=False`; `is_blocked` пока не влияет на вход. |
| Фото профиля | `media/avatars/`. |
