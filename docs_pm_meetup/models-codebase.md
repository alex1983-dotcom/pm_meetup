...
> Снимок для Obsidian и заметок. **Источник правды** — файлы `apps/*/models.py` в репозитории; после изменения моделей этот файл обновляют вручную или перегенерируют.

Документ содержит код моделей: заголовок — модель, ниже — фрагмент класса как в репозитории.

Исходные файлы: `apps/users/models.py`, `apps/core/models.py`, `apps/events/models.py`, `apps/news/models.py`, `apps/content/models.py`, **`apps/materials/models.py`**, **`apps/pages/models.py`**.
...
Формат события в коде — только **офлайн / онлайн** (варианта «гибрид» нет). Цена **`null`/пусто** трактуется как бесплатное участие.
...
## MaterialCategory и Material

Файл: `apps/materials/models.py`

Материалы вынесены в отдельное приложение **`materials`**. Категории — справочник со слагами (кнопки-фильтры на витрине).
...
## Page (статичный контент в Markdown)

Файл: `apps/content/models.py`

Не путать с **`apps.pages.Page`** — страница конструктора для SPA (блоки), см. раздел ниже.
...
## Page, BlockType, PageBlock, BlockItem (конструктор страниц)

Файл: `apps/pages/models.py`

Отдельное приложение **`pages`**: страница задаётся `slug` (например `home`), на неё навешиваются блоки типов из справочника и элементы с текстами. API: `GET /api/pages/<slug>/`.
